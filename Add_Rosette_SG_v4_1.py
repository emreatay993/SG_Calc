...
class SGSelectionForm(Form):
    def __init__(self, num_rows, list__of_num_of_rows):
    ...
    def initialize_components(self, num_rows):
        ...
        for i in list_of_numbers_of_CS_SG_ref_points:
            self.grid.Rows.Add("SG_{}".format(i), placeholder_text)
...
# region Extracting CS reference points, their IDs and names
list_of_obj_of_CS_SG_ref_points = \
    ...
    and Model.CoordinateSystems.Children[i].Suppressed == False]
...
list_of_names_of_CS_SG_ref_points = \
...
# region Extract SG Ref numbers from the names of active CS_SG_Ref objects
# Empty list to hold the numbers, renamed as list_of_numbers_of_CS_SG_ref_points
list_of_numbers_of_CS_SG_ref_points = []

def extract_ref_numbers(names_list):
    # Regular expression pattern to match Ref numbers
    pattern = r"CS_SG_Ref (\d+)"
    
    # List to store extracted Ref numbers
    ref_numbers = []
    ...
    for name in names_list:
        ...
        # If a match is found, add the Ref number to the list
        if match:
            ref_numbers.append(int(match.group(1)))  # Convert to int for numeric values
    
    return ref_numbers
    
list_of_numbers_of_CS_SG_ref_points = extract_ref_numbers(list_of_names_of_CS_SG_ref_points)
# endregion

# region Assign the each rosette SG with their corresponding CAD geometry
class_instance_of_SG_selector = SGSelectionForm(len(list_of_IDs_of_CS_SG_ref_points),list_of_numbers_of_CS_SG_ref_points)
...
for i in list_of_numbers_of_CS_SG_ref_points:
    ...
    geometry_import.Name = "SG_" + str(i) +"_Geometry_Import"
    geometry_import_format = Ansys.Mechanical.DataModel.Enums.GeometryImportPreference.Format.Automatic
    geometry_import_preferences = Ansys.ACT.Mechanical.Utilities.GeometryImportPreferences()
    geometry_import.Import(list_of_selected_CAD_files_for_each_rosette_SG[m], geometry_import_format, geometry_import_preferences)
# endregion
...
list_of_IDs_of_underdefined_SG_grids = [
    ...
    if (list_of_obj_of_bodies[k].Thickness == Quantity(0, 'mm') and 
...
i_iterator = iter(list_of_numbers_of_CS_SG_ref_points)
i=next(i_iterator)
j=1
...
for each_ID_of_underdefined_SG_grids in list_of_IDs_of_underdefined_SG_grids:
    ...
    grid_element_orientation.Name = "Orient_CS_SG_Ch" + str(i) + "_" + str(j)
...
i=next(i_iterator)
m = 0
surface_offset_of_grid_bodies = Quantity(0.05, 'mm')
for sublist in grouped_list_of_SG_grids:
    ...
    part_transform.Name = "SG_" + str(i) + "_Geometry_Placement"
    ...
    part_transform.TargetCoordinateSystem = list_of_obj_of_CS_SG_ref_points[m]
    # endregion
    
    # region Configuring part transformation for translation/orientation to be performed later.
    part_transform = Model.AddPartTransform()
    part_transform.Name = "SG_" + str(i) + "_Position_Orientation_Modification"
    part_transform.Location = body_selection_of_all_channels_of_an_SG
    part_transform.DefineBy = PartTransformationDefinitionType.RotationAndTranslation
    part_transform.CoordinateSystem = list_of_obj_of_CS_SG_ref_points[m]
    # SG grids are moved up from the surface to ensure they always stay above the test piece:
    part_transform.TranslationZ = surface_offset_of_grid_bodies
    # endregion
    ...
    obj_of_mesh_sizing_SG_grid_bodies.Name = "Body_Sizing_SG_" + str(i)
    # endregion
    ...
    connection_group_of_an_SG.Name = "Contacts_SG_" + str(i)
    ...
    try:
        ...
        print("Check Contacts_SG_" + str(i) + " definitions")
