...
# Split the string by space and take the last element, then convert it to an integer and append to a list
for item in list_of_names_of_CS_SG_ref_points:
    try:
        number = int(item.split()[-1])
        list_of_numbers_of_CS_SG_ref_points.append(number)
    except ValueError:
        error_message = "An error occurred during the parsing of \"{item}\" coordinate systems. Ensure that they are named correctly as given (where X is an integer such as 7, representing the gauge number 7 to be placed in the reference point number 7).".format(item=item)
